...
void getgap() {
    ...
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= N; j++) {
            if (visit[i][j] == 1) jh[1] = jh[1] + A[i][j];
            if (visit[i][j] == 2) jh[2] = jh[2] + A[i][j];
            if (visit[i][j] == 3) jh[3] = jh[3] + A[i][j];
            if (visit[i][j] == 4) jh[4] = jh[4] + A[i][j];
            if (visit[i][j] == 5) jh[5] = jh[5] + A[i][j];
        }
